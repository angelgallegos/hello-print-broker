...
if [ "$( docker-machine ls | cut -d ' ' -f1  | grep "${MACHINE_NAME}$" )" != "" ]; then
  ...
  docker-machine create -d virtualbox --virtualbox-hostonly-cidr=192.168.100.1/24 ${MACHINE_NAME}
fi
